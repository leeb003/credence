			...
			if(value.substring(1, 7) == 'iframe'){
				...
				if(value.substring(0, 12) == 'http://vimeo' || value.substring(0,16) == 'http://www.vimeo' || value.substring(0, 13) == 'https://vimeo' || value.substring(0,17) == 'https://www.vimeo'){
					...
					if(code != ''){
						jQuery('#spinner').hide();
						jQuery('.error').remove();
						jQuery('#theimg').attr('src', JSvars.url+'/images/vimeo.jpg');						
						jQuery('#vid_id').val(code);
						jQuery('#vid_img').val(JSvars.url+'/images/vimeo.jpg');
					}
				...
				jQuery('#theimg').remove();
				jQuery('#spinner').hide();
				jQuery('#featured_video_preview').append('<p class="error">This isn\'t a valid embedcode / URL</p>');
