...
<?php
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( 'attachment' == $postdata['post_type'] ) {
					...
					// Mod for local file import to use local images SH LB
                    //$item = basename($remote_url);
                    //$remote_url = get_template_directory_uri() . "/images/demo/$item";
	...
	function fetch_remote_file( $url, $post ) {
		...
		$request = new WP_Http;
		$wp_http = $request->request( $url );

		// fetch the remote url and write it to the placeholder file - copied from wp_get_http in wp functions.php
        $out_fp = fopen($upload['file'], 'w');
        if ( !$out_fp ) {
            // return $headers;
        } else {
            fwrite( $out_fp,  $wp_http['body'] );
            fclose($out_fp);
        }
		...
		if ($wp_http['response']['code'] != '200') {
			@unlink( $upload['file'] );
			//return new WP_Error( 'import_file_error', sprintf( __('Remote server returned error response %1$d %2$s', 'wordpress-importer'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) );
			return new WP_Error( 'import_file_error', sprintf( __('Remote server returned error response %1$d %2$s', 'wordpress-importer'), esc_html($wp_http['headers']['response']), get_status_header_desc($wp_http['headers']['response']) ) );
		}
	...
	//	}
        if ( isset( $wp_http['headers']['content-length'] ) && $filesize != $wp_http['headers']['content-length'] ) {
            @unlink( $upload['file'] );
            return new WP_Error( 'import_file_error', __('Remote file is incorrect size', 'wordpress-importer') );
        }
		...
		if ( isset($wp_http['headers']['x-final-location']) && $wp_http['headers']['x-final-location'] != $url )
            $this->url_remap[$wp_http['headers']['x-final-location']] = $upload['url'];
...
add_action( 'admin_init', 'wordpress_custom_importer_init' );
