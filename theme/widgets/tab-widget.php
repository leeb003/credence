<?php
/*
Tab Widget to add comments, recent posts, tags etc. to.
*/

class sh_tab_widget extends WP_Widget {
	...
	public function __construct() {
        ...
        // ajax functions
        add_action('wp_ajax_sh_widget_content', array(&$this, 'ajax_sh_widget_content'));
        add_action('wp_ajax_nopriv_sh_widget_content', array(&$this, 'ajax_sh_widget_content'));

        // css
        add_action('wp_enqueue_scripts', array(&$this, 'tab_register_scripts'));
        add_action('admin_enqueue_scripts', array(&$this, 'tab_admin_scripts'));

        parent::__construct(
            'sh_tab_widget', // Base ID
            __( 'SH-Themes Tab Widget', 'shcreate'), // Name
            array( 'description' => __('Display popular posts, recent posts, comments, and tags in tabbed format.', 'shcreate'), )
        );
    ...
    function tab_admin_scripts($hook) {
        if ($hook != 'widgets.php')
            return;
        wp_register_script('sh_tab_admin', get_template_directory_uri() . '/theme/admin/js/sh-tabs-admin.js', array('jquery'));  
        wp_enqueue_script('sh_tab_admin');
    }

    function tab_register_scripts() { 
		// JS    
		wp_enqueue_script('tab_widget', get_template_directory_uri() . '/js/tab-widget.min.js', array('jquery'), '', true);     
		wp_localize_script( 'tab_widget', 'sh_tabs', array( 'ajax_url' => admin_url( 'admin-ajax.php' )) );        
	...
	function form( $instance ) {
        ...
        <h4><?php _e('Select Tabs', 'shcreate'); ?></h4>
		...
		<div class="wpt_select_tabs">
			...
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_tags">
				...
				<?php _e( 'Tags Tab', 'shcreate'); ?>
			</label>
		</div>
        <div class="clear"></div>
        
        <h4 class="wpt_tab_order_header"><a href="#"><?php _e('Tab Order', 'shcreate'); ?></a></h4>
        ...
        <div class="wpt_tab_order" style="display: none;">
            ...
            <label class="alignleft" for="<?php echo $this->get_field_id('tab_order'); ?>_tags" style="width: 50%;">
			    ...
			    <?php _e('Tags', 'shcreate'); ?>
            </label>
        </div>
		<div class="clear"></div>
        
        <h4 class="wpt_advanced_options_header"><a href="#"><?php _e('Advanced Options', 'shcreate'); ?></a></h4>
        ...
        <p>
			<label for="<?php echo $this->get_field_id("allow_pagination"); ?>">				
				...
				<?php _e( 'Allow pagination', 'shcreate'); ?>
			</label>
		</p>
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_num'); ?>"><?php _e('Number of posts to show:', 'shcreate'); ?>
				<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id('title_length'); ?>"><?php _e('Title length (words):', 'shcreate'); ?>
				<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb"); ?>">
				...
				<?php _e( 'Show post thumbnails', 'shcreate'); ?>
			</label>
		</p>   
		
		<p class="wpt_thumbnail_size"<?php echo (empty($show_thumb) ? ' style="display: none;"' : ''); ?>>
			<label for="<?php echo $this->get_field_id('thumb_size'); ?>"><?php _e('Thumbnail size:', 'shcreate'); ?></label> 
			<select id="<?php echo $this->get_field_id('thumb_size'); ?>" name="<?php echo $this->get_field_name('thumb_size'); ?>" style="margin-left: 12px;">
				<option value="small" <?php selected($thumb_size, 'small', true); ?>><?php _e('Small', 'shcreate'); ?></option>
				<option value="large" <?php selected($thumb_size, 'large', true); ?>><?php _e('Large', 'shcreate'); ?></option>    
			</select>       
		</p>	
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_date"); ?>">	
				...
				<?php _e( 'Show post date', 'shcreate'); ?>	
			</label>	
		</p>
		...
		<p>		
			<label for="<?php echo $this->get_field_id("show_comment_num"); ?>">		
				...
				<?php _e( 'Show number of comments', 'shcreate'); ?>		
			</label>	
		</p>    
        ...
        <p>
			<label for="<?php echo $this->get_field_id('comment_num'); ?>">
				<?php _e('Number of comments on Comments Tab:', 'shcreate'); ?>
				<br />
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_avatar"); ?>">			
				...
				<?php _e( 'Show avatars on Comments Tab', 'shcreate'); ?>	
			</label>	
		</p>
	...
	function update( $new_instance, $old_instance ) {	
		...
		$instance['tabs'] = isset($new_instance['tabs']) ? $new_instance['tabs'] : '';  
        $instance['tab_order'] = isset($new_instance['tab_order']) ? $new_instance['tab_order'] : '';  
		$instance['allow_pagination'] = isset($new_instance['allow_pagination']) ? $new_instance['allow_pagination'] : '';	
		$instance['post_num'] = isset($new_instance['post_num']) ? $new_instance['post_num'] : '';	
		$instance['title_length'] = isset($new_instance['title_length']) ? $new_instance['title_length'] : '';	
		$instance['comment_num'] =  isset($new_instance['comment_num']) ? $new_instance['comment_num'] : '';		
		$instance['show_thumb'] = isset($new_instance['show_thumb']) ? $new_instance['show_thumb'] : '';     
		$instance['thumb_size'] = isset($new_instance['thumb_size']) ? $new_instance['thumb_size'] : '';		
		...
		$instance['excerpt_length'] = isset($new_instance['excerpt_length']) ? $new_instance['excerpt_length'] : '';	
	...
	function widget( $args, $instance ) {	
        ...
        $available_tabs = array('popular' => __('Popular', 'shcreate'), 
            'recent' => __('Recent', 'shcreate'), 
            'comments' => __('Comments', 'shcreate'), 
            'tags' => __('Tags', 'shcreate'));
            
        array_multisort($tab_order, $available_tabs);
        
		?>	
		<?php echo $before_widget; ?>	
		<div class="sh_tab_widget_content" id="<?php echo $widget_id; ?>_content">	
			<ul class="sh-tabs <?php echo "has-$tabs_count-"; ?>tabs">
	...
	function ajax_sh_widget_content() {     
		$tab = $_POST['tab'];       
		$args = $_POST['args'];  
		...
		switch ($tab) {        
							...
							<?php if ( $show_thumb == 1 ) : ?>			
								<div class="wpt_thumbnail wpt_thumb_<?php echo $thumb_size; ?>">	
                                    <a title="<?php the_title(); ?>" href="<?php the_permalink() ?>">		
    									...
    									<?php else: ?>							
    										<img src="<?php echo get_template_directory_uri() . '/images/tab-widget/'.$thumb_size.'thumb.png'; ?>" alt="<?php the_title(); ?>"  class="wp-post-image" />					
    									<?php endif; ?>
                                    </a>
								</div>				
							<?php endif; ?>					
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>	
								<div class="wpt-postmeta">						
									...
									<?php if ( $show_comment_num == 1 ) : ?>			
										<?php echo comments_number(__('No Comment','shcreate'), __('One Comment','shcreate'), '<span class="comments-number">%</span> '.__('Comments','shcreate'));?>				
									<?php endif; ?>						
								</div> <!--end .entry-meta--> 				
							<?php endif; ?>
						...
						<li>
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>			
								<div class="wpt-postmeta">										
									...
									<?php endif; ?>								
									<?php if ( $show_comment_num == 1 ) : ?>	
										<?php echo comments_number(__('No Comment','shcreate'), __('One Comment','shcreate'), '<span class="comm">%</span> '.__('Comments','shcreate'));?>									
									<?php endif; ?>		
								</div> <!--end .entry-meta--> 		
							<?php endif; ?>
			...
			case "comments":         
				...
				<ul>            
					...
					<?php endforeach; else : ?>           
						<li>                   
							<div class="no-comments"><?php _e('No comments yet.', 'shcreate'); ?></div>        
						</li>                             
    ...
    function tab_pagination($page, $last_page) {  
		...
		<div class="wpt-pagination">     
			<?php if ($page > 1) : ?>               
				<a href="#" class="previous"><span><?php _e('&laquo; Previous', 'shcreate'); ?></span></a>      
			<?php endif; ?>        
			<?php if ($page != $last_page) : ?>     
				<a href="#" class="next"><span><?php _e('Next &raquo;', 'shcreate'); ?></span></a>      
			<?php endif; ?>          
		</div>                   
		<div class="clear"></div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "sh_tab_widget" );' ) );
...
add_action('mts_view_count_after_update', 'sh_tab_update_view_count'); 
...
function ajax_wpt_view_count() {
	// do count
	$post_id = $_POST['id'];
	sh_tab_update_view_count( $post_id );
}
function sh_tab_update_view_count( $post_id ) {
...
function sh_tabs_add_views_meta_for_posts() {
...
function sh_tab_activation() {
	sh_tabs_add_views_meta_for_posts();
}
register_activation_hook( __FILE__, 'sh_tab_activation' );

// unregister Tabs Widget and Tabs Widget v2 - to prevent conflicts
